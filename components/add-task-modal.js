...
export function AddTaskModal({
  ...
  // 1. Internal state to manage visibility for animation
  ...
  // 2. Custom close handler
  // Instead of calling onClose immediately, we set isVisible to false.
  // This triggers AnimatePresence to play the exit animation.
  const handleClose = () => {
    setIsVisible(false);
  };
  ...
  const handleSubmit = () => {
    if (taskTitle.trim()) {
      ...
      handleClose(); // Use handleClose here
    }
  };
  ...
  return (
    // 3. onExitComplete ensures the parent's onClose is called ONLY after animation finishes
    <AnimatePresence onExitComplete={onClose}>
      {isVisible && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 bg-transparent backdrop-blur-sm flex items-end justify-center z-50 lg:bottom-0 bottom-[100px]"
          // 4. Use handleClose for backdrop click
        ...
        >
          <motion.div
            ...
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "100%" }} // This will now play because isVisible controls it
